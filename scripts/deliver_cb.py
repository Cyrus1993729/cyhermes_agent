...
def main():
    ...
    if now.hour >= 17 and now.minute >= 40:
        print("⚠️ 跨境日报生成失败：草稿与存档均缺失或过期（17:00 生成任务未产出，请检查）")
    # 17:15 时段且缺文件 → 静默（生成任务可能还在运行，17:45 会自动重试）
