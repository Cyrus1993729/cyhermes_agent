...
Agent 存在"执行惯性"——反复修改多轮后容易忘记最初要走的完整闭环。两层防线配合：

1. **自动防线（plugin）**：`workflow-tracker` 插件通过 `pre_llm_call` hook 每轮自动注入当前流程状态到 Agent 上下文——Agent 不需要"记得去读"，系统替它读了推进来。
2. **手动防线（skill）**：每个 skill 的入口守卫应先检查追踪文件状态，作为冗余保护。
...
1. **起草前读 `hermes/lessons.md`**：检查本次任务类型是否有历史经验教训可复用，有则融入契约的验收标准。同时检查过往复盘是否调高了"可拆性"门槛（如 Kanban 交付反复出现接缝），如有则优先倾向单模型。
1.5 **判断执行策略（焊死，不跳过）**：
   - **按层判断，不按整任务一刀切。** 几乎每个任务都有两类层：
     - **检索/采集层**：搜数据、读文档、抓取信息
     - **综合/判断层**：分析、推理、成文
   - **三问缝合测试**（对该层的每个候选子块，三问全 Yes 才拆）：
     1. 接口干净吗？（每块产出能独立描述，不需读其他块的中间态）
     2. 产出可独立验证吗？（子块完成能单独验收，不需等其他块）
     3. 块间反馈弱吗？（切开不会切断关键的 feedback loop）
     - 数据检索通常三个都 Yes → **可并行拆**
     - 因果综合/连贯论证通常第三个 No → **不拆，单模型**
   - **结果写入契约**：可以是混合策略（如「检索层并行扇出 + 综合层单模型连贯」），不限于二选一
   - **base-rate 检查**：如果连续 N 次选了同一个答案，抬高举证门槛——写出「如果拆，具体会在哪个缝上崩掉」，写不出说明惯性在替你做决定
   - 判断不准 → 问用户，或让 Opus 帮忙判
...
# 黄金积存金周报（2026-08-17）（1/3）

> 报告日：2026-08-17（周一）｜周期：08-17 ~ 08-23｜数据来源：main.py v3.0 实跑（08:03）+ 实时新闻检索
> **数据场景声明**：全部数据来自 2026-08-17 实时检索（web_search / API / main.py），无模拟推演内容；地缘事件经 CNN、路透、straits.live、Wikipedia 多源交叉验证。

## 【本周要闻】（8/10-8/16）

**1. 8/12 — 美国 7 月 CPI 温和落地，金银大涨**：7 月 CPI 低于预期，缓解对美联储激进加息的担忧；同日中国央行延续创纪录的连续 21 个月购金（goldsilver.com 引述口径）、银价双引擎同启、霍尔木兹海峡处于事实关闭状态（CNN 8/13 确认"自 2 月底美以对伊开战以来被德黑兰有效关闭"；路透 8/14 报道海峡通行受限于美伊竞相主张）。goldsilver.com 将当日行情归因于五重因素（媒体叙事口径，贡献度未经量化拆分；"超买"实为过热警示而非驱动力）。来源：https://goldsilver.com/industry-news/goldsilver-news/gold-silver-market-news-digest-august-3-2026 ；交叉验证：https://www.cnn.com/2026/08/13/world/live-news/iran-war-trump 、https://www.reuters.com/world/middle-east/hormuz-shipping-traffic-capped-amid-competing-claims-us-iran-2026-08-14 、https://straits.live

**2. 8/13 — 美国 7 月 PPI 出炉，能源价格冲击缓解**：通胀压力受控（伊朗冲突相关能源价格冲击在 7 月缓解），市场对 9 月加息概率定价降至约 1/3。来源：https://www.tradingeconomics.com/commodity/gold

**3. 8/14 — 金价收于 $4,375 上方（现货口径），连续第二周周线上涨**：XAU 现货 8/14 收 $4,375.50（+0.60%，TradingEconomics CFD 口径），近一月 +7.76%，同比 +31.15%；1 月历史高点 $5,608.35。对应 COMEX GC=F 期货 $4,427.30（含约 +1.2% 升水，与 D4 校验一致）。来源：https://www.tradingeconomics.com/commodity/gold ；https://highfxrebates.com/blog/post/weekly-market-recap-august-3-9-2026

**4. 8/14 — 世界黄金协会：中国 7 月官方部门购金强劲**：WGC 确认 7 月官方购买强劲（Ray Jia 报告）。来源：https://www.gold.org/goldhub/gold-focus/2026/08/china-gold-market-update-strong-official-sector-buying-july

**5. 8/12 — 地缘性反弹的可持续性遭质疑**：金价自 1 月高点 $5,608 一度回落约 28%（8 月初低点约 $4,020）后反弹至 $4,375（较 1 月高点仍低约 22%），分析师提示缺乏利率/美元配合的地缘反弹难持续。来源：https://www.investing.com/analysis/gold-outlook-geopolitical-selloff-raises-questions-over-safehaven-demand-200684572

## 【量化分析】（main.py v3.0 完整输出）

```
🔶 黄金积存金 周报 v3.0    周期: 08-17 ~ 08-23 | 报告日: 2026-08-17
📊 最新金价: $4,427.30 (COMEX GC=F) | ¥958.27/克 (XAU×USDCNY 换算, as_of 08-16)
    USDCNY: 6.7322 | 纯 XAU×USDCNY 换算（无 SGE 锚，仅供参考）
📈 综合评分: 50.6/100 ⚪ | 置信度: 低置信 (53/100)
🎯 本周建议: 维持观望 | 定投: 50% | 体制: 混合体制
⚠️ 数据哨兵 WARN: 隐含汇率偏差 +1.21% (内外价差或数据滞后)

【数据新鲜度】宏观环境 08-25更新(23天前)→权重60% | 央行行为 08-08(9天前)→80%
  上海金: 纯 XAU×USDCNY 换算（无 SGE 锚）

【本周研判】混合体制；dxy（利多）、gsr（利多）、real_yield（利空）为主要驱动，
dxy 最突出——美元指数 99.6（近1月走弱1.1%）。短期指标（macd、cot）利空与中长期
利多背离；etf_flow、境内ETF流量利多但可能为短暂反弹。多空均衡，低置信(53/100)，
建议降低操作幅度。定投50%，回落至公允价值下方3-7%可补入剩余额度。

【信号全景图】
▲ 结构性锚: 央行购金 pboc_reserves +1.0 | fiscal_gpr 0.0 | resilience 0.0
● 中期驱动: real_yield -2.0 (TIPS 2.39%上行) | dxy +1.5 (美元走弱) | fed_policy -0.5
  ma_position 0.0 ($4427>50周均线$4384,<200日均线$4489) | trend_strength 0.0 (ADX=24)
  gsr +1.0 (金银比68) | gold_oil -1.0 (金油比54高位) | 人民币汇率 -1.0 (20日升值0.6%)
  fair_value/gold_m2/gold_spx 0.0 | decoupling_proxy 0.0 | 贵金属分化 0.0
△ 短期扰动: macd -1.0 (周线死叉 MACD=-77.7) | rsi 0.0 (53) | cot -1.0 (净多218k手偏重)
  etf_flow +2.0 (GLD 20日流入9.0%) | 上海溢价 0.0 (境内正常) | 境内ETF流量 +2.0 (份额20日+78.1%)
  油金共振 +0.2 (Z=0.47) | 波动率溢价 +0.3 (GVZ=24, 超额+2)

【分项评分（含衰减）】
  宏观环境(20%): -0.45 🟠⚡ | 技术趋势(15%): -0.25 | 情绪仓位(10%): +0.30
  估值水平(15%): -0.20 | 央行行为(20%): +0.40 🟡⚡ | 境内需求(10%): +0.30
  地缘政治(10%): +0.18    ⚡=数据陈旧权重衰减

💰 建议在 ¥944-953/克 等待回调后买入（当前AU9999 ¥958.3，公允价值 ¥958.3，偏离 +0.0%）
✅ 周报已保存: output/weekly_08-17_08-23.json
```
...
本周（8/10-8/14）金价连续第二周收涨，周五现货收于 $4,375.50（+0.60%，现货口径仅作参考；核心量化口径为 COMEX GC=F 期货价，契约口径），近一月 +7.76%。核心驱动：**① 通胀预期边际缓和**——7 月 CPI/PPI 显示能源冲击缓解，9 月加息概率定价降至约 1/3；**② 美元走弱**——美元指数 99.6、近 1 月 -1.1%（模型 dxy +1.5，中期最突出利多）；**③ 央行与避险买盘**——中国央行连续 21 个月增持（WGC 确认 7 月强劲）、霍尔木兹海峡关闭推升油金共振（GVZ=24，超额 +2）。

主要矛盾：实际利率上行（TIPS 2.39%，-2.0 分）对冲美元走弱；短期 MACD 死叉 + CFTC 投机净多 218k 手偏重 vs GLD ETF 20 日流入 9.0%、境内 ETF 份额 +78.1%。综合评分 50.6/100（低置信 53/100——模型已连续 11 周滞留中性/微多区，属**结构性低置信**），多空均衡、方向博弈期。

### 4.2 短期预判（1-4 周）

- **模型信号**：混合体制、方向博弈期；结构性低置信（连续 11 周中性区，中性档 50% 定投额度与置信度无关，见 4.3）；判断有效期至下次美联储会议/重大数据前（8/19 FOMC 7 月纪要、9 月中 FOMC 为节点）。
- **大行观点**：渣打（8/3）Q4 均价 $4,650（自 $4,500 上调）、有望重测 $5,000；花旗 2026Q4 $4,500、2027H1 $5,000；WGC ±5% 区间波动为主、破位催化剂酝酿中；SSGA $4,000-4,500 整固、30% 概率触 $5,000。
- **综合判断（推断）**：1-4 周高位震荡，区间参考 **$4,300-4,480**——上界 200 日均线 $4,489 附近（突破即趋势转强）；下界 $4,300 为 50 周均线 $4,384 下方约 2% 缓冲带（推断设定，周线收盘跌破即确认中期转弱）。**核心假设**：① 9 月 FOMC 不加息；② 霍尔木兹局势不急剧升级/缓和；③ 人民币不快速单边升值（**首要承重假设**——境内需求项 -1.0 已计价温和升值，快速升值将吞噬人民币金价涨幅）。
- **风险提示（推断）**：双向替代情景——① FOMC 纪要偏鹰/加息概率回升 → 实际利率压制、跌破 $4,300 转弱；② 反方 1：央行购金放缓 → pboc_reserves 支撑弱化；③ 反方 2：美元技术性反弹 → 最强利多 dxy 反向施压；④ 反方 3：霍尔木兹恢复通行 → 油金共振/波动率溢价回吐；⑤ 反方 4（拥挤交易反转）：CFTC 净多 218k 手偏重，地缘缓和或加息预期升温触发多单集体平仓有踩踏风险；⑥ 反之央行购金延续+美元续弱 → 突破 $4,480 打开上行。
- **假设失效→动作映射（推断）**：① 9 月意外加息/纪要偏鹰 → 停止加仓、定投降至 25-30%；② 霍尔木兹快速恢复通行 → 兑现部分浮盈；③ 人民币快速升值 → 暂缓人民币计价加仓。
- **地缘数据局限（盲区标注）**：霍尔木兹状态基于 CNN/路透/straits.live（8/16 更新，Day 169），但随伊朗-阿曼谈判随时可能变化；模型输入（GVZ、油金共振 Z=0.47）反映市场定价而非事件本身，事件变化可能先于模型信号。

### 4.3 操作建议

- **定投策略**：维持月度定投 **50%**（模型语义：半额定投·不主动加码）。低置信说明：50% 为中性档固定档位、与置信度无关（scorer.py 中性档不读置信度）；如需体现低置信可自行下调至 25-30%（推断）。
- **加仓条件**：a) 短线买入区 **¥944-953/克**（当前 AU9999 ¥958.3、公允价值 ¥958.3、偏离 +0.0%，即回调 0.6%-1.5%，来源：main.py）——对应美元约 **$4,361-4,403**（USDCNY 6.7322 静态换算，汇率变动需重算）；b) 公允价值下方 3-7%（¥891-929/克）补入剩余 50%——**⚠️ 本周参数下该带不可触发**（见下条）。
- **⚠️ 两档指令冲突处理（推断，阻断级提示）**：深度补仓带 ¥891-929/克（=958.3×[0.93,0.97]）与止损参考位 $4,300（≈¥930.8/克）重叠且方向相反——**补仓带整体位于止损位下方**。**执行规则：技术止损位优先**——周线收盘跌破 $4,300 即确认中期转弱，补仓带作废、停止补入转为观望；**本周参数下深度补仓带不可触发**（公允价值与止损位之间无有效买入空间），仅在公允价值上移或止损位下移后重新生效。
- **减仓/观望**：站稳 $4,480（200 日均线）前不加码；周线 MACD 死叉延续且 CFTC 净多继续攀升则暂缓加仓（推断）。
- **长期持仓**：央行购金（pboc_reserves +1.0 结构性锚）与美元走弱构成中期支撑，长期定投逻辑未破坏（来源：main.py）。
- ⚠️ 以上均为参考建议，不构成投资决策依据。

## 【数据校验】（D4）（3/3）

| 校验项 | 数值 | 判定 |
|:---|:---|:---|
| 金价 API | GC=F $4,427.30 vs XAU 现货 $4,374.50（gold-api，8/17 00:04Z）→ **+1.21%** | ✅ ≤2%（正常 contango） |
| 汇率换算 | main.py ¥958.27 vs 独立换算 ¥947.9（$4,374.50÷31.1035×CNH 6.74）→ **+1.09%** | ✅ ≤3% |
| 现货快照 | $4,375.50（TE 8/14 收盘）vs $4,374.50（gold-api 快照），差 $1=0.02% | ✅ 来源/时点差异 |
| 月涨幅口径 | 模型 decoupling_proxy 10.3%（GC=F 窗口）vs 要闻 7.76%（现货） | ✅ 已注明 |
| SGE 新鲜度 | 5 天旧（8/12），自动 GC=F×USDCNY 换算兜底；上海溢价 +0.0% 为缺失填充值（非实测） | ✅ 契约预期场景 |
| FRED 完整性 | DFII10/M2SL/T10Y2Y 全获取；fredapi 已装；.env 有 key | ✅ 完整 |
| 综合评分勾稽 | 50.6 ⇔ composite 0.0228（`to_100_scale=(score+2)/4×100`，scorer.py L259-261；权重经体制调整→归一化→新鲜度衰减，面板为衰减前权重不可直接手算；consecutive=11 周中性区不抑制） | ✅ 代码级验证 |
| 数据衰减 | 宏观 23 天旧（60%）、央行 9 天旧（80%）；⚠️ dxy 等日频子项被 M2 陈旧度连坐降权（代码工单） | ✅ 已披露 |

**来源标注**：要闻附日期+URL（地缘事件四源交叉验证）；量化数据来自 main.py v3.0 实跑（08-17 08:03）；推断项均已标注（推断）。

---

🔍 收尾自检：
✅ 步骤完整（契约 D1-D4 全部交付：要闻 5 条含日期+URL / main.py 完整输出 / 复盘+预判+建议 / 六项数据校验全过阈值）
✅ 来源清晰（要闻 URL、量化数据 main.py、推断标注（推断）、地缘四源交叉验证、数据场景声明）
✅ 审查 CONDITIONAL→PASS（L1 3 轮未 PASS：R1 CONDITIONAL→R2/R3 FAIL，R4 被内置计数器阻断（8h 上限），三轮发现项已全部修复；转 Opus L2 复审：R1 CONDITIONAL（2 处阻断级：补仓带 vs 止损位冲突、加仓带缺美元双轨，+6 项）→ R2 CONDITIONAL（1 处阻断级已修）→ **R3 PASS**；L18 规则记录）
✅ 已存档 → hermes_output/2026-08-17_gold_weekly.md + contracts/archive/gold_weekly_2026-08-17/（contract+deliverable），index.md 已更新
✅ 复盘已存 → references/任务复盘/2026-08-17_gold_weekly.md（新增 lessons L16/L17/L18）

⚠️ 遗留提示（不阻断交付）：① 数据陈旧——宏观 23 天旧、SGE 5 天旧，境内需求类结论置信度已相应下调；② main.py/scorer.py 代码工单 3 项（"3-7% 补仓"硬编码与暂停档撞车、面板标题与有效权重不一致、macro 新鲜度误伤日频子项），建议后续周修复。
