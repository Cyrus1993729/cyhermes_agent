...
✅ **Ad-hoc 定向验证完成（非套件）** — `data_fetcher.py` 锚日修复验证：

- **方式**：验证脚本经 `tempfile.mkstemp(prefix="hermes-verify-", suffix=".py")` 生成 OS-safe 临时路径（`C:\Users\ADMINI~1\AppData\Local\Temp\hermes-verify-3bhn7pof.py`），在 gold_analyzer 工作目录下执行，跑完即清理（临时副本与暂存文件均已删除）。
- **结果（exit 0，全部断言通过）**：
  1. **Bug 复现**：旧 `<=` 比较选中 GLD 404.92（08-12）而非锚日 398.96（08-13）→ 2 日窗口、方向失真（-0.85%）——证明测试可判别新旧行为；
  2. **修复生效**：`_extrapolate_shanghai` 外推估值 **¥940.60 == 期望 ¥940.60**（936.17 × 401.48/398.96 × 6.7322/6.7428），锚日当根 K 线被正确选中；
  3. **哨兵数学**：隐含汇率 6.6613 vs 实际 6.7322，偏差 **-1.05%**（±2% 内、软警告带），与 12:38 实跑 -1.09% 一致；
  4. **tz-aware 守卫**：fresh yfinance 路径（America/New_York 索引）经 normalize+localize 得到同值。
- **说明**：gold_analyzer 无规范化测试/构建命令，此为针对性 ad-hoc 验证，非 suite green；修复同时已由真实运行（哨兵 FAIL -2.54% → WARN -1.09% 正常出报）交叉印证。
