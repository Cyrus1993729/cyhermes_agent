...
- **触发太软**：短语"任务收尾时触发"在实际对话中约等于不触发（已验证：review_log 长期为空）。Agent 必须在每次交付物完成时主动问自己"我刚才交付了什么？"，然后立即创建契约 + 运行审查，不等"阶段切换"。
...
## 配对 Skill
- **sprint-contract**：任务开始时出契约，收尾时 l1-review 用它逐条比对。
- 两 skill 共同构成完整闭环：事前定标准 → 干活 → 事后逐条审。
