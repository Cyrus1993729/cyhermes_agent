...
**🆕 P2 自动打勾（本步骤完成 → 自动记录）**：收尾自检全部通过后，运行：
```
...
**解决：三明治防线。**
1. **自动层**：`workflow-tracker` 插件通过 `pre_llm_call` hook 每轮自动注入流程状态——Agent 不需要"记得去读"追踪文件。
2. **用户哨兵**：双触发词。**"走流程"** → 契约+闸门+执行；**"收尾"** → 审查+存档+复盘。
3. **手动层**：每个 skill 入口守卫检查追踪文件（冗余保护）。
