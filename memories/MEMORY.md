...
§
TG已配置(token在Desktop/各类api key/telegram bot token.txt)。API走代理7897，NO_PROXY禁含api.telegram.org。TG池耗尽修复：_drain_send_connections清_request[1]。
§
...
§
模型架构: 主=deepseek-v4-pro，fallback+delegation=kimi-k3(kimi-coding-cn, CN站)。Kimi下拉框曾不显→根因kimi-coding默认国际站与CN key不匹配→改kimi-coding-cn+KIMI_CN_API_KEY解决。Qwen3.7Max=custom:qwen-bailian(DashScope, sk-ws- key)。
§
DeepSeek API不支持Vision：deepseek-chat只接受text类型，image_url→400。×hs图文帖Vision步骤在DeepSeek下失效。回退：正文密集帖直接基于desc；稀疏帖需Qwen等替代。